...
>>> from logmodpy import run, var, fact
>>> from logmodpy.assoccomm import eq_assoccomm as eq
>>> from logmodpy.assoccomm import commutative, associative
...
from logpy.unification.utils import transitive_get as walk
...
def makeops(op, lists):
    ...
    >>> from logmodpy.assoccomm import makeops
...
def partition(tup, part):
    ...
    >>> from logmodpy.assoccomm import partition
...
def groupsizes_to_partition(*gsizes):
    """
    >>> from logmodpy.assoccomm import groupsizes_to_partition
...
def eq_assoc(u, v, eq=core.eq, n=None):
    ...
    >>> from logmodpy import run, var, fact
    >>> from logmodpy.assoccomm import eq_assoc as eq
...
def eq_comm(u, v, eq=None):
    ...
    >>> from logmodpy import run, var, fact
    >>> from logmodpy.assoccomm import eq_comm as eq
    >>> from logmodpy.assoccomm import commutative, associative
...
def eq_assoccomm(u, v):
    ...
    >>> from logmodpy.assoccomm import eq_assoccomm as eq
    >>> from logmodpy.assoccomm import commutative, associative
    >>> from logmodpy import fact, run, var
